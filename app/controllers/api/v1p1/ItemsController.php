...
class ItemsController extends BaseController {
	...
	public function getSpawn($count, $userId, $lat, $lng, $anywhere = true)
	{
		...
		if ($lat != '' && $lng != '') {
			...
			if ($anywhere == false) {
				...
				$nearests = Location::where('id', 0)->get()->toArray();
				$nearest = $nearests[0];
			}
	...
	public function getReceive($userId, $couponCode, $lat, $lng) {
		...
		// if item_id == 4 | 5 then get voucher
		$voucher_id = null;
		$voucher = null;
		if ($receivedItem['item_id'] == 4 || $receivedItem['item_id'] == 5) {
			$voucher = Voucher::whereNull('user_tracking_id')->take(1)->get()->first();
			if ($voucher != null) $voucher_id = $voucher->id;
		}
		...
		if ($voucher != null) {
			$voucher->user_tracking_id = $userTracking->id;
			$voucher->save();
		}
		...
		if ($voucher != null) {
			$userTracking->voucher = $voucher->toArray();
		} else {
			$userTracking->voucher = null;
		}
	...
	public function getVouchers($userId) {
		if ($userId == '') return;
		$myVouchers = Voucher::whereNotNull('user_tracking_id')->with(array('userTracking.spawnedItem' => function($query) use ($userId)
			{
